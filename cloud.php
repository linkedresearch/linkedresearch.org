      ...
      <article about="" typeof="schema:Article">
        ...
        <div datatype="rdf:HTML" id="content" property="schema:description">
          ...
          <section id="about" rel="schema:hasPart" resource="#about">
            ...
            <div datatype="rdf:HTML" property="schema:description">
              <p>This article has an inbox to receive <cite><a href="https://www.w3.org/TR/ldn/">Linked Data Notifications</a></cite> (<abbr title="Linked Data Notifications">LDN</abbr>) about scholarly activities, eg. publication of scholarly articles (in any status), <cite><a href="https://www.w3.org/TR/annotation-model">Web Annotation</a></cite> (like peer reviews, replies), citations, call for contributions, scientific observations, funding information etc. Practically anything along the lines of the Linked Research <a href="rfc#challenges">challenges</a>. The notifications refer to actual works which are expected to follow the <cite><a about="" href="https://www.w3.org/DesignIssues/LinkedData" rel="cito:givesSupportTo">Linked Data design principles</a></cite>. <strong>Only mentions about complete works and activities that are free and open are welcome</strong> for inclusion in the graph. The notifications are used towards constructing the <q>cloud</q>.</p>

              <h3 id="why-notifications">Why notifications?</h3>
              <p>It is possible to achieve this without having to create accounts or fill out awkward forms. LORC will be dynamically autogenerated by client-side JavaScript. Notifications are Linked Data resources, so they can be discovered and consumed by applications which comply with that stack of technologies. We will deal with <q>RDF spam</q> if and when we have to.</p>

              <h3 id="what-are-notifications">What are notifications?</h3>
              <p>LDN notifications tend to be information snippets about some object or activity. The graph is normally dynamically constructed (although your agent may cache it) so make sure that the important parts of what it refers to (like a peer review in response to an article) is both accessible and contains full content. We do not automatically verify whether an object contains the full article, but if it does not and we find out, we will remove it 😉. Stored notifications can be discovered and accessed from the inbox (via <code>ldp:contains</code>). Each will have its own persistent URL.</p>
...
<code>'</code></pre>
              ...
              <dl>
                ...
                <dd>The graph is <a href="https://creativecommons.org/licenses/by/4.0/">CC BY 4.0</a>. Notifications have their own license - whatever assigned by the sender. Notification payloads without a license will be assigned with <a href="https://creativecommons.org/publicdomain/zero/1.0/">CC0 1.0 Universal</a> license.</dd>
